...
import { AUCTION_EXPLANATION, AUCTION_LABEL, BAND_EXPLANATION, PROVISIONAL_EXPLANATION } from "./priceSeries";
...
export async function buildPriceWorkbook(rows) {
  ...
  aboutSheet.getCell("A1").value = "How low, average, and peak are worked out";
  ...
  const columns = [
    ...
    { header: "Trailing 7d p33 (£/MWh)", key: "p33", width: 20 },
    { header: "Trailing 7d p67 (£/MWh)", key: "p67", width: 20 },
  ];
  if (hasProvisional) {
  ...
  for (const row of sorted) {
    const rowData = {
      ...
      pence: round2(gbpToPence(row.price_gbp)),
      gbp: round2(row.price_gbp),
      band: BAND_LABEL[row.band] ?? row.band,
      p33: round2(row.trailing_7d_p33),
      p67: round2(row.trailing_7d_p67),
    };
    if (hasProvisional) {
