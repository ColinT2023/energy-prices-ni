...
export const BAND_EXPLANATION =
  "Each half hour is compared to all the half-hourly prices from the last 7 days. The cheapest third are shown as low, the priciest third as peak and the rest as typical.";

// Display label only — the underlying band value stays "low"/"average"/
// "peak" (the SQL case statement in ni_prices_banded, and aggregateDaily's
// band string below, both unchanged), this is just what a user sees.
// Shared by the table and the Excel export so a future rename touches one
 ...
 * quoted by AUCTION_EXPLANATION below, by the Help page's own glossary
 * entries (imported directly), and by lib/priceRange.js's
 * notPublishedMessage() and TOMORROW_TABLE_CAPTION, so it can't drift
 * into four independently-worded copies of the same fact.
 *
 * Deliberately an under-promise, not a best-fit description of the real
 * distribution: real published-time data across 227 SEM-DA reports
 * (every day since 31 Dec 2025, weekends and bank holidays included, no
 * gaps) has a median/mode of 11:55am UK time — before this window even
 * starts — with p95 at 12:01pm and p99 at 12:43pm. The window's floor
 * (12pm) sits safely past the typical publish time, so "check from 12"
 * usually finds the data already there; its ceiling (1pm) comfortably
 * covers even the rare late outlier. A window actually centred on
 * 11:55am would describe the real distribution more accurately, but
 * risks someone citing "usually there by around midday" at 12:05 and
 * being wrong on an unlucky day — worse than this window's occasional
 * over-caution.
 */
export const TYPICAL_PUBLISH_TIME = "between 12 and 1pm";
...
export const AUCTION_EXPLANATION = `SEM-DA is the day ahead auction, usually published ${TYPICAL_PUBLISH_TIME} the day before delivery and setting the first price for every half hour of the next day. SEM-IDA1, SEM-IDA2, and SEM-IDA3 are three further intraday auctions held later on the day itself, each repricing the same settlement periods with more up to date information. A higher number means a more recent revision, so SEM-IDA3 — where a period has one — is the closest to real time.`;
...
export function gbpToPence(priceGbp) {
  // price_gbp is £/MWh (wholesale); the site displays p/kWh (retail-scale).
  // 1 MWh = 1000 kWh, so £/MWh -> p/kWh is a straight divide by 10.
  return priceGbp / 10;
}

// price_gbp already carries 2 decimal places in the source data, and
// p/kWh is derived directly from it (÷10) so the same precision survives
// — every surface that displays a price should go through these two (or
// roundPrice, for places that need a number rather than a display
// string, like the Excel export) rather than its own toFixed/Math.round,
// which is how three separate components ended up independently disagreeing
// on precision in the first place.
export function formatPence(priceGbp) {
  return gbpToPence(priceGbp).toFixed(2);
}

export function formatGbp(priceGbp) {
...
/**
 * Same averaging approach as aggregateDaily (mean price, mean of the
 * trailing_7d_p33/p67 cutoffs each row was already judged against,
 * re-banded from those averages, provisional-if-any), bucketed by hour
 * instead of calendar day — used only by the chart's Both series to thin
 * today's ~48 half-hourly intraday points down to ~24 hourly ones. Both
 * needs real detail preserved (unlike All time's daily collapse, the
 * whole point of this view is comparing today's actual shape against
 * tomorrow's day-ahead curve), so this halves the point count rather
 * than flattening it to one point.
 *
 * Buckets by flooring each row's raw UTC timestamp to the top of the
 * hour, not by reconstructing a London-local hour string — London's
 * offset from UTC is always a whole number of hours (0 or +1, never a
 * half-hour step), so an hour boundary in UTC is always exactly an hour
 * boundary in London-local time too, on both sides of every DST
 * transition. That makes this simpler than aggregateDaily's ymd-based
 * bucketing without losing correctness: no timezone-aware reconstruction
 * needed, just integer division.
 */
export function aggregateHourly(seriesRows) {
  const byHour = new Map();
  for (const row of seriesRows) {
    const hourStart = Math.floor(new Date(row.datetime).getTime() / 3600000) * 3600000;
    let bucket = byHour.get(hourStart);
    if (!bucket) {
      bucket = { hourStart, sumPrice: 0, sumP33: 0, sumP67: 0, count: 0, provisional: false };
  ...
  return [...byHour.values()]
    .map((b) => {
      ...
      const band = avgPrice < avgP33 ? "low" : avgPrice > avgP67 ? "peak" : "average";
      return {
        datetime: new Date(b.hourStart).toISOString(),
        price_gbp: avgPrice,
        band,
        periodCount: b.count,
        provisional: b.provisional,
