 ...
 *
 * "today" and "tomorrow" are both bound to exactly that one London
 * calendar day — expressed as dayRange(ymd) rather than hand-rolling a
 * forward step, since dayRange/customRange already handle the DST-safe
 ...
 * had `to: null` (open-ended) and would silently pick up tomorrow's rows
 * the moment they existed (e.g. day-ahead auctioned this afternoon, or via
 * the provisional feed) — confirmed live before this fix landed.
 *
...
export function presetRange(scope) {
  ...
  if (scope === "tomorrow") return dayRange(shiftYmd(todayYmd, 1));
